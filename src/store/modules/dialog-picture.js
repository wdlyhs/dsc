...
const mutations = {
    [SET_DIALOG_PICTURE](state, o) {
        if (localStorage.getItem("aPicture")) {
            JSON.parse(localStorage.getItem("aPicture")).forEach(v => {
    ...
    [ONE_MORE_SELECT](state, o) {
        ...
        if (sOneOrMore == "one") {
            ...
            if (localStorage.getItem("aPicture")) {
                nActiveNum = JSON.parse(localStorage.getItem("aPicture")).length
            }
            if (nActiveNum >= nResidueLength) {
                if (!state.pictureList[o.listIndex].bActive) {
                    let msg = i18n.t('lang.components_added_most') + nMaxLength + i18n.t('lang.components_img')
                    Message.error(msg)
                }
            ...
            if (state.pictureList[o.listIndex].bActive) {
                if (localStorage.getItem("aPicture")) {
                    ...
                    localStorage.setItem("aPicture", JSON.stringify(aPicture))
                } else {
                    aPicture.push(picture)
                    localStorage.setItem("aPicture", JSON.stringify(aPicture))
                }
